...
def template_context(request: Request, **kwargs) -> dict:
    ...
    ctx = dict(kwargs)
    # Always generate a CSRF token — the middleware sets the signed cookie on
    # every GET (keyed to current user_id, which is "" for unauthenticated).
    # Every form needs a matching hidden field, including the setup wizard.
