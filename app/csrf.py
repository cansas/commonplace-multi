...
def template_context(request: Request, **kwargs) -> dict:
    """Build a template context dict with csrf_token, user_theme, and vapid_public_key."""
    ctx = dict(kwargs)
    # Use the token pre-generated by CSRFMiddleware (if available) so the
    # form's hidden field and the cookie value are identical. On POST
    # responses the middleware doesn't set request.state.csrf_token, so
    # fall back to reusing the existing cookie token — avoids generating
    # a new token that won't match the cookie on the next submission.
    token = getattr(request.state, "csrf_token", None)
    if not token:
        cookie_token = request.cookies.get(CSRF_COOKIE_NAME)
        if cookie_token and verify_csrf_token(cookie_token, request.session):
            token = cookie_token
        else:
    ...
    if "vapid_public_key" not in ctx:
        try:
            ...
            ctx["vapid_public_key"] = get_vapid_public_key()
        except Exception:
            pass
    return ctx
